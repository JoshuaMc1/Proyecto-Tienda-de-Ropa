...
public class funciones {
   ...
   /* public void eliminar(String sentencia){
    ...
    public void validaTexto(TextField tf, int longitud){ //Metodo para validar que solo se ingresen letras y espacios
        UnaryOperator<TextFormatter.Change> filter = change ->{ 
            if (change.getControlNewText().matches("[a-zA-Z ]*") && change.getControlNewText().length() <= longitud){ /*si el texto que se encuentra en la caja de texto es una letra o 
   ...
   public void validaNumeros(TextField tf,  int longitud){
       UnaryOperator<TextFormatter.Change> filter = change ->{ 
            if (change.getControlNewText().matches("[0-9]*") && change.getControlNewText().length() <= longitud){ /*si el texto que se encuentra en la caja de texto es un numero*/
                return change; 
            }
            return null; //si no no devuelve nada o sea no permite que se ingrese el valor tecleado
        };
   ...
   public void manejaLongitudTxt(TextField tf, int longitud){ 
       UnaryOperator<TextFormatter.Change> filter = change ->{ 
            if (change.getControlNewText().length() <= longitud){ /*si la longitud del texto es menor a la longitud deseada por el usuario*/
